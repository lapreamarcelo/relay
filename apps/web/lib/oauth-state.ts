...
export function createOAuthState(userId: string, brandId: string | null, flow: OAuthFlow): string {
...
export function verifyOAuthState(token: string, cookieToken: string | undefined, expectedFlow: string, expectedUserId: string): OAuthStatePayload {
  ...
  if (!payload.userId || (payload.brandId !== null && typeof payload.brandId !== "string") || !payload.flow || !payload.nonce || !payload.expiresAt) throw new Error("OAuth state payload was incomplete");
