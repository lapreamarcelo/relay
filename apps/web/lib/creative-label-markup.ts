...
import { creativeLabelHeight, labelFonts, wrapCreativeLabel } from "./creative-labels.ts";

function escapeXml(value: string): string {
  return value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&apos;");
}

export function creativeLabelsMarkup(labels: CreativeLabel[], width = 1080, height = 1920): string {
  ...
  return `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">${elements}</svg>`;
}
