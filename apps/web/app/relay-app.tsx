...
function BrandsView({ onBrandCreated, onBrandUpdated, onBrandDeleted, onAccountsAssigned }: { onBrandCreated: (brand: Brand) => void; onBrandUpdated: (brand: Brand) => void; onBrandDeleted: (id: string) => void; onAccountsAssigned: (assignments: { id: string; brandId: string }[]) => void }) {
  ...
  return <div className="page page-enter"><div className="inline-heading"><div><h2>Brands</h2><p>A brand is a workspace for one project’s accounts, timezone, and publishing defaults.</p></div><button className="primary-button" onClick={() => setCreating(true)}><Plus /> New brand</button></div><div className="brand-explainer"><span><LayoutGrid /></span><div><b>Keep publishing identities separate</b><p>Each connected account belongs to one brand. Edit a brand to add or move Instagram, Facebook, TikTok, and YouTube accounts.</p></div></div>{brands.length === 0 ? <Empty title="No brands yet" body="Create your first brand, then choose which connected accounts belong to it." /> : <div className="brand-grid">{brands.map((brand) => <article key={brand.id}><div className="brand-cover" style={{ "--brand": brand.color } as React.CSSProperties}><BrandMark brandId={brand.id} size="large" /></div><div><h3>{brand.name}</h3><p>{accounts.filter((account) => account.brandId === brand.id).length} accounts · {brand.timezone}</p><div className="provider-stack">{accounts.filter((account) => account.brandId === brand.id).map((account) => <ProviderIcon id={account.provider} key={account.id} />)}</div></div><BrandActions brand={brand} onEdit={() => setEditing(brand)} onDelete={() => setDeleting(brand)} /></article>)}</div>}{creating && <BrandModal onClose={() => setCreating(false)} onSaved={saved(onBrandCreated)} />}{editing && <BrandModal brand={editing} onClose={() => setEditing(null)} onSaved={saved(onBrandUpdated)} />}{deleting && <DeleteBrandModal brand={deleting} onClose={() => setDeleting(null)} onDeleted={onBrandDeleted} />}</div>;
}
...
function Composer({ onClose, onCreate }: { onClose: () => void; onCreate: (post: RelayPost) => void }) {
  ...
  const [media, setMedia] = useState<{ name: string; url: string; type: "image" | "video" } | null>(null);
  ...
  const canSubmit = Boolean(hasContent && brandId && selected.length > 0 && youtubeReady && (!schedule || scheduledAt));
  const addMedia = async (file: File) => {
    ...
    try {
      ...
      setMedia({ name: file.name, url, type: file.type.startsWith("video/") ? "video" : "image" });
    } catch (cause) { setMediaError(cause instanceof Error ? cause.message : "Media upload failed"); }
  ...
  const submit = () => {
    ...
    onCreate({ id: `post-${Date.now()}`, brandId, text, mediaType: media?.type ?? "none", mediaUrl: media?.url, status: schedule ? "scheduled" : "published", scheduledAt: schedule ? new Date(scheduledAt).toISOString() : undefined, publishedAt: schedule ? undefined : new Date().toISOString(), targets });
    onClose();
  };
...
export default function RelayApp({ user, initialBrands, initialAccounts, initialNow }: { user: { name: string; email: string; role: string }; initialBrands: Brand[]; initialAccounts: SocialAccount[]; initialNow: string }) {
  ...
  const [composer, setComposer] = useState(false); const [command, setCommand] = useState(false); const [menu, setMenu] = useState(false); const [theme, setTheme] = useState("light"); const [toast, setToast] = useState(""); const [toastTone, setToastTone] = useState<"success" | "error">("success");
