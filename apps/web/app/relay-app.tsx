...
function PublishingDefaultsSettings({ defaults, onSaved }: { defaults: PublishingDefaults; onSaved: (value: PublishingDefaults) => void }) {
  ...
  return <><p>Choose the starting values used when you create a new post. Existing posts and templates keep their own settings.</p><div className="provider-defaults">
    ...
    <article><header><ProviderIcon id="facebook" selected /><div><b>Facebook</b><small>Choose the starting format for video.</small></div></header><div className="provider-default-fields"><label>Video<select aria-label="Facebook video default" value={draft.facebook.videoPublishType} onChange={(event) => { setSaved(false); setDraft((current) => ({ ...current, facebook: { videoPublishType: event.target.value as PublishingDefaults["facebook"]["videoPublishType"] } })); }}><option value="reel">Reel</option><option value="feed">Feed post</option></select></label></div><p>Images and multi-photo posts always start as Feed posts.</p></article>
    <article><header><ProviderIcon id="tiktok" selected /><div><b>TikTok</b><small>Privacy and interaction defaults.</small></div></header><div className="provider-default-fields"><label>Visibility<select aria-label="TikTok visibility default" value={draft.tiktok.privacyLevel} onChange={(event) => { setSaved(false); setDraft((current) => ({ ...current, tiktok: { ...current.tiktok, privacyLevel: event.target.value as PublishingDefaults["tiktok"]["privacyLevel"] } })); }}><option value="SELF_ONLY">Only me</option><option value="MUTUAL_FOLLOW_FRIENDS">Friends</option><option value="FOLLOWER_OF_CREATOR">Followers</option><option value="PUBLIC_TO_EVERYONE">Everyone</option></select></label></div><div className="provider-default-toggles"><label><input type="checkbox" checked={draft.tiktok.allowComments} onChange={(event) => { setSaved(false); setDraft((current) => ({ ...current, tiktok: { ...current.tiktok, allowComments: event.target.checked } })); }} /> Comments</label><label><input type="checkbox" checked={draft.tiktok.allowDuet} onChange={(event) => { setSaved(false); setDraft((current) => ({ ...current, tiktok: { ...current.tiktok, allowDuet: event.target.checked } })); }} /> Duet</label><label><input type="checkbox" checked={draft.tiktok.allowStitch} onChange={(event) => { setSaved(false); setDraft((current) => ({ ...current, tiktok: { ...current.tiktok, allowStitch: event.target.checked } })); }} /> Stitch</label></div></article>
...
function Composer({ onClose, onSave, publishingDefaults, initialMedia = null, initialPost = null, initialText = "", initialBrandId, mode = "create" }: { onClose: () => void; onSave: (post: RelayPost, mode: ComposerMode) => Promise<boolean>; publishingDefaults: PublishingDefaults; initialMedia?: ComposerMedia | null; initialPost?: RelayPost | null; initialText?: string; initialBrandId?: string; mode?: ComposerMode }) {
  ...
  useEffect(() => {
    ...
    if (!facebookTypeTouched.current) setFacebookType(media?.type === "video" ? publishingDefaults.facebook.videoPublishType : "feed");
  ...
  const isInstagramReelPreview = activePreviewProvider === "instagram" && instagramType === "reel";
  ...
  return <div className="composer-layer"><button className="modal-scrim" onClick={onClose} aria-label="Close composer" /><section className="composer" role="dialog" aria-modal="true" aria-label={mode === "edit" ? "Edit post" : "Create post"}>
    ...
    <div className="composer-body"><main>
      ...
      {selectedProviders.length > 0 && <section className="platform-settings"><div className="platform-settings-heading"><span className="field-label">Network captions</span><p>Leave a field empty to use the shared caption.</p></div><div className="network-variants">{selectedProviders.map((provider) => <label key={provider}><span><ProviderIcon id={provider} />{providerRegistry.get(provider).name}</span><textarea value={variants[provider] ?? ""} onChange={(event) => setVariants((current) => ({ ...current, [provider]: event.target.value }))} placeholder={`Use shared caption (${text.length} characters)`} /></label>)}</div><div className="platform-settings-heading"><span className="field-label">Platform settings</span><p>These values are stored separately for each destination.</p></div>
        ...
        {selectedProviders.includes("facebook") && <div className="platform-card"><div className="platform-card-title"><ProviderIcon id="facebook" /><span><b>Facebook</b><small>{mediaCount > 1 ? "Multi-photo posts publish to the Page feed in slide order." : "Page publishing options."}</small></span></div><div className="platform-grid"><label>Publish as<select disabled={mediaCount > 1} value={facebookType} onChange={(event) => { facebookTypeTouched.current = true; setFacebookType(event.target.value as typeof facebookType); }}><option value="feed">Feed post</option>{mediaCount <= 1 && <option value="reel">Reel</option>}</select></label><label>Link URL <span>optional</span><input type="url" value={facebookLink} onChange={(event) => setFacebookLink(event.target.value)} placeholder="https://…" /></label></div></div>}
    ...
    </main><aside className="preview-panel"><div className="preview-head"><span>{activePreviewProvider ? `${providerRegistry.get(activePreviewProvider).name} preview` : "Preview"}</span><div className="segmented compact" aria-label="Preview format">{(["feed", "mobile"] as const).map((preview) => <button key={preview} className={previewMode === preview ? "active" : ""} aria-pressed={previewMode === preview} onClick={() => setPreviewMode(preview)}>{preview[0].toUpperCase() + preview.slice(1)}</button>)}</div></div>{selectedProviders.length > 0 && <div className="preview-provider-tabs" aria-label="Preview destination">{selectedProviders.map((provider) => <button key={provider} className={activePreviewProvider === provider ? "active" : ""} aria-pressed={activePreviewProvider === provider} onClick={() => setPreviewProvider(provider)}><ProviderIcon id={provider} /><span>{providerRegistry.get(provider).name}</span></button>)}</div>}<div className={`social-preview ${previewMode} ${activePreviewProvider ? `provider-${activePreviewProvider}` : ""} ${isInstagramReelPreview ? "format-reel" : ""}`}>{previewMode === "mobile" && <div className="phone-status"><b>9:41</b><span>● ◒ ▰</span></div>}{isInstagramReelPreview ? <><div className="reel-stage">{media ? <div className="reel-media">{media.type === "video" ? <video src={media.previewUrl} muted playsInline loop autoPlay /> : <img src={media.previewUrl} alt={media.name} />}{mediaBusy && <span><LoaderCircle className="spin" />Uploading…</span>}</div> : <div className="preview-placeholder"><span className="preview-glyph"><i /><i /></span><p>Your media will appear here</p></div>}<div className="reel-topbar"><b>Reels</b><Camera /></div><div className="reel-actions" aria-hidden="true"><span><Heart /></span><span><MessageCircle /></span><span><Send /></span><span><MoreHorizontal /></span></div><div className="reel-copy"><div><BrandMark brandId={brandId} size="small" /><b>{brands.find((brand) => brand.id === brandId)?.name ?? "Unassigned"}</b></div><p>{previewText}</p><small><Music2 /> Original audio</small></div></div>{previewMode === "mobile" && <div className="phone-home-indicator" />}</> : <><div className="preview-account"><BrandMark brandId={brandId} size="small" /><span><b>{brands.find((brand) => brand.id === brandId)?.name ?? "Unassigned"}</b><small>{activePreviewProvider ? providerRegistry.get(activePreviewProvider).name : "Preview"} · just now</small></span><MoreHorizontal /></div>{media ? <div className="preview-media">{media.type === "video" ? <video src={media.previewUrl} muted controls /> : <img src={media.previewUrl} alt={media.name} />}{mediaBusy && <span><LoaderCircle className="spin" />Uploading…</span>}</div> : <div className="preview-placeholder"><span className="preview-glyph"><i /><i /></span><p>Your media will appear here</p></div>}<div className="preview-actions"><span>♡</span><span>◯</span><span>⌁</span></div><p>{previewText}</p>{previewMode === "mobile" && <div className="phone-home-indicator" />}</>}</div><div className="validation"><h4>Destination check</h4>{selectedAccounts.map((account) => { const issue = validationIssues.find((item) => item.provider === account.provider || !item.provider); return <div key={account.id}><ProviderIcon id={account.provider} /><span>{providerRegistry.get(account.provider).name}</span>{issue ? <em title={issue.message}><CircleAlert /> {issue.message}</em> : <em className="ready"><Check /> Ready</em>}</div>; })}</div></aside></div>
