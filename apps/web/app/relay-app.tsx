...
import { type Brand, type Campaign, type PostTemplate, type ProviderId, type ProviderPostSettings, type RelayPost, type SocialAccount, type PostStatus } from "@relay/core";
...
function Composer({ onClose, onSave, initialMedia = null, initialPost = null, initialText = "", initialBrandId, mode = "create" }: { onClose: () => void; onSave: (post: RelayPost, mode: ComposerMode) => Promise<boolean>; initialMedia?: ComposerMedia | null; initialPost?: RelayPost | null; initialText?: string; initialBrandId?: string; mode?: ComposerMode }) {
  const brands = useBrands();
  const accounts = useAccounts();
  ...
  useEffect(() => { setSelected((current) => current.filter((id) => { const account = accounts.find((item) => item.id === id); return account && !unavailableReason(account); })); if (mediaCount > 1) { setInstagramType("feed"); setFacebookType("feed"); } }, [media?.type, mediaCount, accounts]);
  ...
  const applyTemplate = (templateId: string) => {
    ...
    const instagram = template.settings.instagram; if (instagram?.kind === "instagram") setInstagramType(instagram.publishType);
    const facebook = template.settings.facebook; if (facebook?.kind === "facebook") { setFacebookType(facebook.publishType); setFacebookLink(facebook.linkUrl ?? ""); }
