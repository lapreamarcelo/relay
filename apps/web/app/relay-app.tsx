...
import { descendantMediaProjectIds, flattenMediaProjects, mediaProjectOptionLabel } from "../lib/media-projects";
...
function Sidebar({ active, onChange, mobileOpen, onClose, user, onLogout }: { active: View; onChange: (view: View) => void; mobileOpen: boolean; onClose: () => void; user: { name: string; role: string }; onLogout: () => void }) {
  ...
  return <>
    ...
    <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
      ...
      <div className="sidebar-sponsor"><GoldSponsor /></div>
...
function FolderDestinationPicker({ options, value, onChange, includeRoot = false, includeUnsorted = false }: { options: Array<{ id: string; name: string; depth?: number; count?: number }>; value: string; onChange: (value: string) => void; includeRoot?: boolean; includeUnsorted?: boolean }) {
  const [query, setQuery] = useState("");
  const normalizedQuery = query.trim().toLowerCase();
  const choices = [
    ...(includeRoot ? [{ id: "root", name: "Root folders", depth: 0, count: undefined }] : []),
    ...(includeUnsorted ? [{ id: "unfiled", name: "Unsorted", depth: 0, count: undefined }] : []),
    ...options,
  ].filter((option) => !normalizedQuery || option.name.toLowerCase().includes(normalizedQuery));
  return <div className="folder-destination-picker"><div className="folder-destination-search"><Search /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search destination folders…" aria-label="Search destination folders" />{query && <button type="button" onClick={() => setQuery("")} aria-label="Clear destination folder search"><X /></button>}</div><div className="folder-destination-list" role="listbox" aria-label="Destination folders">{choices.length === 0 ? <p className="folder-destination-empty">No folders match “{query.trim()}”.</p> : choices.map((choice) => <button type="button" role="option" aria-selected={value === choice.id} className={value === choice.id ? "active" : ""} style={{ "--folder-depth": choice.depth ?? 0 } as React.CSSProperties} onClick={() => onChange(choice.id)} key={choice.id}><span className="folder-destination-icon">{choice.id === "root" ? <FolderOpen /> : <Folder />}</span><span><b>{choice.name}</b><small>{choice.id === "root" ? "Top-level folders" : choice.id === "unfiled" ? "Keep outside a folder" : `${choice.count ?? 0} item${choice.count === 1 ? "" : "s"}`}</small></span>{value === choice.id && <Check />}</button>)}</div></div>;
}

function MoveMediaModal({ items, projects, value, busy, error, onChange, onClose, onConfirm }: { items: MediaObject[]; projects: MediaProject[]; value: string; busy: boolean; error: string; onChange: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  const multiple = items.length > 1;
  const canMoveTo = (destination: string) => items.some((item) => mediaProjectId(item.key) !== destination);
  const choices = projects.filter((project) => canMoveTo(project.id)).map((project) => ({ id: project.id, name: project.name, count: project.count }));
  return <div className="modal-layer"><button className="modal-scrim" disabled={busy} onClick={onClose} aria-label="Cancel moving media" /><form className="prompt-modal move-media-modal" role="dialog" aria-modal="true" aria-labelledby="move-media-title" onSubmit={(event) => { event.preventDefault(); onConfirm(); }}><header><div><p className="eyebrow">Media library</p><h2 id="move-media-title">Move {multiple ? `${items.length} files` : "media"} to a folder</h2><p>Choose a destination from your current folders.</p></div><button type="button" className="icon-button" disabled={busy} onClick={onClose} aria-label="Close"><X /></button></header><div className="move-media-file"><span><FolderInput /></span><div><b>{multiple ? `${items.length} selected files` : items[0]?.name}</b><small>{multiple ? items.slice(0, 3).map((item) => item.name).join(", ") + (items.length > 3 ? ` +${items.length - 3} more` : "") : "Choose its new R2 folder"}</small></div></div><label>Destination folder</label><FolderDestinationPicker options={choices} value={value} onChange={onChange} includeUnsorted />{error && <p className="auth-error" role="alert">{error}</p>}<footer><button type="button" className="secondary-button" disabled={busy} onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy || !value}>{busy ? <LoaderCircle className="spin" /> : <FolderInput />}{busy ? `Moving ${items.length}…` : `Move ${multiple ? `${items.length} files` : "file"}`}</button></footer></form></div>;
}

function MoveMediaFolderModal({ project, projects, value, busy, error, onChange, onClose, onConfirm }: { project: MediaProject; projects: MediaProject[]; value: string; busy: boolean; error: string; onChange: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  const options = flattenMediaProjects(projects.filter((candidate) => candidate.kind === project.kind)).filter(({ project: candidate }) => candidate.id !== project.id && !descendantMediaProjectIds(projects, project.id).has(candidate.id));
  const choices = options.map(({ project: candidate, depth }) => ({ id: candidate.id, name: candidate.name, depth, count: candidate.count }));
  return <div className="modal-layer"><button className="modal-scrim" disabled={busy} onClick={onClose} aria-label="Cancel moving folder" /><form className="prompt-modal move-media-modal" role="dialog" aria-modal="true" aria-labelledby="move-media-folder-title" onSubmit={(event) => { event.preventDefault(); onConfirm(); }}><header><div><p className="eyebrow">Media library</p><h2 id="move-media-folder-title">Move folder</h2><p>Choose a parent folder, or keep it at the root.</p></div><button type="button" className="icon-button" disabled={busy} onClick={onClose} aria-label="Close"><X /></button></header><div className="move-media-file"><span><FolderInput /></span><div><b>{project.name}</b><small>{project.count} item{project.count === 1 ? "" : "s"} · {project.kind === "music" ? "music" : "media"} folder</small></div></div><label>Destination folder</label><FolderDestinationPicker options={choices} value={value} onChange={onChange} includeRoot />{error && <p className="auth-error" role="alert">{error}</p>}<footer><button type="button" className="secondary-button" disabled={busy} onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy}>{busy ? <LoaderCircle className="spin" /> : <FolderInput />}{busy ? "Moving folder…" : "Move folder"}</button></footer></form></div>;
}

function MoveMediaFoldersModal({ folders, projects, value, busy, error, onChange, onClose, onConfirm }: { folders: MediaProject[]; projects: MediaProject[]; value: string; busy: boolean; error: string; onChange: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  const selectedIds = new Set(folders.map((folder) => folder.id));
  const selectedDescendants = new Set(folders.flatMap((folder) => [...descendantMediaProjectIds(projects, folder.id)]));
  const options = flattenMediaProjects(projects.filter((candidate) => candidate.kind === folders[0]?.kind)).filter(({ project: candidate }) => !selectedIds.has(candidate.id) && !selectedDescendants.has(candidate.id)).map(({ project: candidate, depth }) => ({ id: candidate.id, name: candidate.name, depth, count: candidate.count }));
  return <div className="modal-layer"><button className="modal-scrim" disabled={busy} onClick={onClose} aria-label="Cancel moving folders" /><form className="prompt-modal move-media-modal" role="dialog" aria-modal="true" aria-labelledby="move-media-folders-title" onSubmit={(event) => { event.preventDefault(); onConfirm(); }}><header><div><p className="eyebrow">Media library</p><h2 id="move-media-folders-title">Move {folders.length} folders</h2><p>Choose a parent folder for the selected folders.</p></div><button type="button" className="icon-button" disabled={busy} onClick={onClose} aria-label="Close"><X /></button></header><div className="move-media-file"><span><FolderInput /></span><div><b>{folders.length} folders selected</b><small>{folders.slice(0, 2).map((folder) => folder.name).join(", ")}{folders.length > 2 ? ` +${folders.length - 2} more` : ""}</small></div></div><label>Destination folder</label><FolderDestinationPicker options={options} value={value} onChange={onChange} includeRoot />{error && <p className="auth-error" role="alert">{error}</p>}<footer><button type="button" className="secondary-button" disabled={busy} onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy || !value}>{busy ? <LoaderCircle className="spin" /> : <FolderInput />}{busy ? "Moving folders…" : "Move folders"}</button></footer></form></div>;
}
...
function MediaView({ onCompose, publishingDefaults = defaultPublishingDefaults, demoMode = false, initialItems = [], initialMusicItems = [], initialProjects = [] }: { onCompose: (media: ComposerMedia) => void; publishingDefaults?: PublishingDefaults; demoMode?: boolean; initialItems?: MediaObject[]; initialMusicItems?: MediaObject[]; initialProjects?: MediaProject[] }) {
  ...
  const [createOpen, setCreateOpen] = useState(false); const [projectName, setProjectName] = useState(""); const [createParentId, setCreateParentId] = useState<string | null>(null); const [projectBusy, setProjectBusy] = useState(false); const [projectError, setProjectError] = useState("");
  ...
  const [folderSelectionMode, setFolderSelectionMode] = useState(false); const [selectedFolderIds, setSelectedFolderIds] = useState<string[]>([]); const [moveFolderSelection, setMoveFolderSelection] = useState<MediaProject[]>([]); const [moveFolderParentId, setMoveFolderParentId] = useState("root"); const [moveFolderError, setMoveFolderError] = useState("");
  ...
  const loadProjects = async () => {
    const requestVersion = ++projectsLoadVersion.current;
    try { const response = await fetch("/api/v1/media/projects", { cache: "no-store" }); const payload = await response.json() as { data?: MediaProject[]; error?: string }; if (!response.ok) throw new Error(payload.error || "Could not load media projects"); if (requestVersion === projectsLoadVersion.current) setProjects(payload.data ?? []); }
  ...
  const load = async (pageCursor: string | null) => {
    ...
    try {
      ...
      if (assetKind === "media" && mediaType !== "all") params.set("mediaType", mediaType);
  ...
  useEffect(() => { void load(cursor); }, [cursor, projectId, assetKind, mediaType]);
  ...
  const createProject = async () => {
    ...
    try { const response = await fetch("/api/v1/media/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: projectName, kind: assetKind, parentId: createParentId }) }); const payload = await response.json() as { data?: MediaProject; error?: string }; if (!response.ok || !payload.data) throw new Error(payload.error || "Could not create the project"); projectsLoadVersion.current += 1; setProjects((current) => [...current.filter((project) => project.id !== payload.data!.id), payload.data!].sort((first, second) => first.name.localeCompare(second.name))); setProjectId(payload.data.id); setCursor(null); setHistory([]); setCreateOpen(false); setProjectName(""); setCreateParentId(null); }
  ...
  const canMoveFolderTo = (sourceId: string, destinationId: string) => destinationId === "root" || sourceId !== destinationId && !descendantMediaProjectIds(projects, sourceId).has(destinationId);
  const moveFolderTo = async (sourceId: string, destinationId: string) => {
    const source = projects.find((project) => project.id === sourceId);
    if (!source || !canMoveFolderTo(sourceId, destinationId) || projectBusy) return;
    setProjectBusy(true); setProjectError(""); setError("");
    try {
      const response = await fetch("/api/v1/media/projects", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: sourceId, parentId: destinationId === "root" ? null : destinationId }) });
      const payload = await response.json() as { data?: MediaProject; error?: string };
      if (!response.ok || !payload.data) throw new Error(payload.error || "Could not move the folder");
      setProjects((current) => current.map((project) => project.id === payload.data!.id ? payload.data! : project));
      setMoveProject(null); setSequenceNotice(`${source.name} moved ${destinationId === "root" ? "to the root" : `inside ${projects.find((project) => project.id === destinationId)?.name ?? "the selected folder"}`}.`);
    } catch (cause) { const message = cause instanceof Error ? cause.message : "Could not move the folder"; setProjectError(message); setError(message); }
    finally { setProjectBusy(false); }
  };

  const openMoveFolder = (project: MediaProject) => { setMoveProject(project); setMoveParentId(project.parentId ?? "root"); setProjectError(""); };
  const toggleFolderSelection = (project: MediaProject) => setSelectedFolderIds((current) => {
    if (current.includes(project.id)) return current.filter((id) => id !== project.id);
    if (current.some((id) => descendantMediaProjectIds(projects, id).has(project.id))) return current;
    const descendants = descendantMediaProjectIds(projects, project.id);
    return [...current.filter((id) => !descendants.has(id)), project.id];
  });
  const openMoveSelectedFolders = () => { const folders = selectedFolderIds.map((id) => projects.find((project) => project.id === id)).filter((project): project is MediaProject => Boolean(project)); if (!folders.length) return; setMoveFolderSelection(folders); setMoveFolderParentId("root"); setMoveFolderError(""); };
  const moveSelectedFolders = async () => {
    if (!moveFolderSelection.length || !moveFolderParentId) return;
    setProjectBusy(true); setMoveFolderError(""); setError("");
    try {
      const failed: MediaProject[] = []; const updated = new Map<string, MediaProject>();
      for (const folder of moveFolderSelection) {
        const response = await fetch("/api/v1/media/projects", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: folder.id, parentId: moveFolderParentId === "root" ? null : moveFolderParentId }) });
        const payload = await response.json() as { data?: MediaProject; error?: string };
        if (!response.ok || !payload.data) failed.push(folder); else updated.set(folder.id, payload.data);
      }
      if (updated.size) setProjects((current) => current.map((project) => updated.get(project.id) ?? project));
      if (failed.length) { setMoveFolderSelection(failed); setSelectedFolderIds(failed.map((folder) => folder.id)); setMoveFolderError(`${moveFolderSelection.length - failed.length} of ${moveFolderSelection.length} folders moved. Try again.`); return; }
      setSelectedFolderIds([]); setFolderSelectionMode(false); setMoveFolderSelection([]); setSequenceNotice(`${moveFolderSelection.length} folders moved.`);
    } catch (cause) { setMoveFolderError(cause instanceof Error ? cause.message : "Could not move the selected folders"); }
    finally { setProjectBusy(false); }
  };
  const startFolderDrag = (event: React.DragEvent<HTMLButtonElement>, project: MediaProject) => { if (projectBusy) return; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", project.id); setDraggedProjectId(project.id); setFolderDropTargetId(null); };
  const overFolder = (event: React.DragEvent<HTMLButtonElement>, project: MediaProject) => { const sourceId = draggedProjectId ?? event.dataTransfer.getData("text/plain"); if (!sourceId || !canMoveFolderTo(sourceId, project.id)) return; event.preventDefault(); event.dataTransfer.dropEffect = "move"; setFolderDropTargetId(project.id); };
  const dropFolder = (event: React.DragEvent<HTMLButtonElement>, project: MediaProject) => { const sourceId = draggedProjectId ?? event.dataTransfer.getData("text/plain"); if (!sourceId || !canMoveFolderTo(sourceId, project.id)) return; event.preventDefault(); setDraggedProjectId(null); setFolderDropTargetId(null); void moveFolderTo(sourceId, project.id); };
  ...
  const chooseProject = (next: string) => { setProjectId(next); setFolderQuery(""); setCursor(null); setHistory([]); };
  const switchAssetKind = (next: "media" | "music") => { setAssetKind(next); setMediaType("all"); setSelectedItems([]); setFolderQuery(""); setSelectedFolderIds([]); setFolderSelectionMode(false); if (demoMode) setItems(next === "music" ? initialMusicItems : initialItems); setProjectId("all"); setCursor(null); setHistory([]); };
  const chooseMediaType = (next: "all" | "image" | "video") => { setMediaType(next); setCursor(null); setHistory([]); };
  const visibleProjects = projects.filter((project) => (project.kind ?? "media") === assetKind);
  const visibleProjectTree = flattenMediaProjects(visibleProjects);
  const normalizedFolderQuery = folderQuery.trim().toLowerCase();
  const filteredProjectTree = normalizedFolderQuery ? visibleProjectTree.filter(({ project }) => project.name.toLowerCase().includes(normalizedFolderQuery)) : visibleProjectTree;
  // Keep the opened folder and its ancestors available in the compact strip.
  const activeFolderPath = new Set<string>();
  let activeFolder = visibleProjects.find((project) => project.id === projectId);
  while (activeFolder && !activeFolderPath.has(activeFolder.id)) {
    activeFolderPath.add(activeFolder.id);
    activeFolder = visibleProjects.find((project) => project.id === activeFolder?.parentId);
  }
  const displayedProjectTree = folderView === "grid" || normalizedFolderQuery ? filteredProjectTree : filteredProjectTree.filter(({ project }) => !project.parentId || activeFolderPath.has(project.id));
  useEffect(() => {
    activeFolderRef.current?.scrollIntoView({ block: "nearest", inline: "nearest" });
  }, [projectId, folderView, normalizedFolderQuery, projects]);
  ...
  const openMove = (moveSelection: MediaObject[]) => {
    const firstDestination = ["unfiled", ...visibleProjects.map((project) => project.id)].find((destination) => moveSelection.some((item) => mediaProjectId(item.key) !== destination)) ?? "";
    setMoveItems(moveSelection); setMoveProjectId(firstDestination); setMoveError("");
  };

  const move = async () => {
    if (!moveItems.length || !moveProjectId) return;
    const pending = [...moveItems];
    setBusyKey("move-selection"); setMoveError(""); setError("");
    try {
      const failed: MediaObject[] = [];
      for (const item of pending) {
        const response = await fetch("/api/v1/media", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key: item.key, projectId: moveProjectId, kind: assetKind }) });
        if (!response.ok) failed.push(item);
      }
      const movedKeys = new Set(pending.filter((item) => !failed.some((failure) => failure.key === item.key)).map((item) => item.key));
      setSelectedItems((current) => current.filter((item) => !movedKeys.has(item.key)));
      await Promise.all([load(cursor), loadProjects()]);
      if (failed.length) { setMoveItems(failed); setMoveError(`${pending.length - failed.length} of ${pending.length} files moved. ${failed.length} could not be moved; try again.`); }
      else { setMoveItems([]); setSequenceNotice(`${pending.length} file${pending.length === 1 ? "" : "s"} moved to ${moveProjectId === "unfiled" ? "Unsorted" : visibleProjects.find((project) => project.id === moveProjectId)?.name ?? "the selected folder"}.`); }
    } catch (cause) { setMoveError(cause instanceof Error ? cause.message : "Could not move the selected files"); }
    finally { setBusyKey(null); }
  };
  ...
  const shownItems = demoMode && assetKind === "media" && mediaType !== "all" ? items.filter((item) => mediaKind(item.name) === mediaType) : items;
  const childProjects = projectId !== "all" && projectId !== "unfiled" ? visibleProjects.filter((project) => project.parentId === projectId) : [];
  const hasVisibleContents = shownItems.length > 0 || childProjects.length > 0;
  const canScheduleSelection = selectedItems.length > 0 && selectedItems.every((item) => { const kind = mediaKind(item.name); return kind === "image" || kind === "video"; });
  ...
  return <div className="page page-enter"><div className="inline-heading"><div><h2>Asset library</h2><p>Keep videos, images, and licensed music in named R2 folders that you and your agents can reuse.</p></div><button className="primary-button" disabled={busyKey === "upload"} onClick={() => inputRef.current?.click()}>{busyKey === "upload" ? <LoaderCircle className="spin" /> : <Upload />} {busyKey === "upload" ? uploadProgress : `Upload to ${selectedName}`}</button><input ref={inputRef} className="visually-hidden" type="file" multiple accept={assetKind === "music" ? "audio/*" : "image/*,video/*"} onChange={(event) => void uploadFiles(event.target.files)} /></div>
    ...
    <section className="media-projects"><header><div><p className="eyebrow">R2 folders</p><h3>{selectedProject ? selectedProject.name : "Choose a reusable asset folder"}</h3></div><div className="media-project-header-actions">{selectedProject && <><button className="secondary-button" onClick={() => openMoveFolder(selectedProject)}><FolderInput /> Move folder</button><button className="secondary-button" onClick={() => { setRenameProject(selectedProject); setRenameProjectName(selectedProject.name); setProjectError(""); }}><Pencil /> Rename folder</button><button className="danger-outline-button" onClick={() => { setDeleteProject(selectedProject); setProjectError(""); }}><Trash2 /> Delete folder</button></>}<button className="secondary-button" onClick={() => { setFolderSelectionMode((current) => !current); setSelectedFolderIds([]); }}>{folderSelectionMode ? <X /> : <Check />}{folderSelectionMode ? "Done selecting" : "Select folders"}</button>{selectedFolderIds.length > 0 && <button className="primary-button" onClick={openMoveSelectedFolders}><FolderInput /> Move selected</button>}<button className="secondary-button media-folder-preview-trigger" onClick={() => setFolderView((current) => current === "grid" ? "strip" : "grid")} aria-pressed={folderView === "grid"}>{folderView === "grid" ? <List /> : <Grid2X2 />}{folderView === "grid" ? "Compact folders" : "Preview all folders"}</button><button className="secondary-button" onClick={() => { setProjectName(""); setCreateParentId(projectId !== "all" && projectId !== "unfiled" ? projectId : null); setProjectError(""); setCreateOpen(true); }}><Plus /> New folder</button></div></header><div className="media-project-toolbar"><label className="media-folder-search"><Search /><span className="visually-hidden">Search folders</span><input type="search" value={folderQuery} onChange={(event) => setFolderQuery(event.target.value)} placeholder="Search folders…" aria-controls="media-folder-list" />{folderQuery && <button type="button" onClick={() => setFolderQuery("")} aria-label="Clear folder search"><X /></button>}</label></div><div id="media-folder-list" className={`media-project-list ${folderView}`}>{!normalizedFolderQuery && <><button className={`media-system-folder${projectId === "all" ? " active" : ""}`} onClick={() => chooseProject("all")}><span><Images /></span><b>{assetKind === "music" ? "All music" : "All media"}</b><small>{assetKind === "music" ? "Every audio track" : "Every visual asset"}</small></button><button className={`media-system-folder unsorted${projectId === "unfiled" ? " active" : ""}`} onClick={() => chooseProject("unfiled")}><span><FolderOpen /></span><b>Unsorted</b><small>Existing and loose files</small></button></>}{displayedProjectTree.map(({ project, depth }) => { const parentName = project.parentId ? visibleProjects.find((candidate) => candidate.id === project.parentId)?.name : undefined; return <button ref={projectId === project.id ? activeFolderRef : undefined} aria-current={projectId === project.id ? "location" : undefined} title={project.name} className={`media-folder-node ${(folderSelectionMode ? selectedFolderIds.includes(project.id) : projectId === project.id) ? "active" : ""}${folderDropTargetId === project.id ? " folder-drop-target" : ""}`} style={{ "--folder-depth": depth } as React.CSSProperties} draggable={!projectBusy} onDragStart={(event) => startFolderDrag(event, project)} onDragOver={(event) => overFolder(event, project)} onDrop={(event) => dropFolder(event, project)} onDragEnd={() => { setDraggedProjectId(null); setFolderDropTargetId(null); }} onClick={() => folderSelectionMode ? toggleFolderSelection(project) : chooseProject(project.id)} key={project.id}><span><Folder /></span><b>{project.name}</b><small>{project.count} item{project.count === 1 ? "" : "s"} · {parentName ? `Inside ${parentName}` : "Root folder"}</small></button>; })}</div>{normalizedFolderQuery && filteredProjectTree.length === 0 && <div className="media-folder-empty"><Search /><span><b>No folders match “{folderQuery.trim()}”</b><small>Try a shorter name or clear the search.</small></span><button className="secondary-button" onClick={() => setFolderQuery("")}>Clear search</button></div>}</section>
    {assetKind === "media" && <div className="media-type-filter"><span>Show</span><div className="segmented compact" aria-label="Filter media by type">{([['all', 'All'], ['image', 'Images'], ['video', 'Videos']] as const).map(([value, label]) => <button className={mediaType === value ? "active" : ""} aria-pressed={mediaType === value} onClick={() => chooseMediaType(value)} key={value}>{label}</button>)}</div></div>}
    {sequenceNotice && <div className="slideshow-notice success">{sequenceNotice}<button onClick={() => setSequenceNotice("")}><X/></button></div>}
    {selectedItems.length > 0 && <section className="media-selection-bar"><b>{selectedItems.length} selected</b><span>Selection stays active while you browse folders, filters, and pages.</span><button className="secondary-button" onClick={() => setSelectedItems([])}>Clear</button><button className="secondary-button" disabled={visibleProjects.length === 0} onClick={() => openMove(selectedItems)}><FolderInput/> Move to folder</button>{canScheduleSelection && <button className="primary-button" onClick={() => setSequenceOpen(true)}><CalendarDays/> Schedule sequence</button>}</section>}
    {error && <div className="media-error"><CircleAlert />{error}<button onClick={() => void load(cursor)}>Retry</button></div>}
    {loading ? <div className="media-loading"><LoaderCircle className="spin" />Loading media from R2…</div> : !hasVisibleContents ? <Empty title={mediaType === "all" ? `${selectedName} is empty` : `No ${mediaType === "image" ? "images" : "videos"} in ${selectedName}`} body={assetKind === "music" ? "Upload an audio track here, or choose another music folder." : mediaType === "all" ? "Upload an image or video here, or choose another media folder." : `Choose another type or upload ${mediaType === "image" ? "an image" : "a video"}.`} /> : <div className="media-grid">{childProjects.map((project) => <article className="media-folder-card" key={project.id}><button className="media-folder-card-main" onClick={() => chooseProject(project.id)}><span><FolderOpen /></span><div><b>{project.name}</b><small>{project.count} item{project.count === 1 ? "" : "s"} · Folder</small></div><ChevronRight /></button><button className="secondary-button" onClick={() => chooseProject(project.id)}>Open folder</button></article>)}{shownItems.map((item) => { const kind = mediaKind(item.name); const busy = busyKey === item.key || busyKey === "move-selection"; const selected = selectedItems.some((selectedItem) => selectedItem.key === item.key); return <article className={selected ? "selected" : ""} key={item.key}><div className="media-image">{kind === "image" ? <img src={item.url} alt={item.name} loading="lazy" /> : kind === "video" ? <video src={item.url} preload="metadata" muted /> : kind === "audio" ? <div className="media-audio"><Music2/><audio src={item.url} controls preload="metadata"/></div> : <span className="media-file"><FileIcon /></span>}<button className="media-select" aria-label={selected ? `Deselect ${item.name}` : `Select ${item.name}`} aria-pressed={selected} disabled={busy} onClick={() => toggleSelected(item)}>{selected && <Check/>}</button><span>{kind === "video" ? <Video /> : kind === "image" ? <ImageIcon /> : kind === "audio" ? <Music2/> : <FileIcon />}{kind}</span><div className="media-actions">{(kind === "image" || kind === "video") && <button className="icon-button" disabled={busy} onClick={() => setPreviewItem(item)} aria-label={`Preview ${item.name}`} title="Preview"><Eye /></button>}<button className="icon-button" disabled={busy || visibleProjects.length === 0} onClick={() => openMove([item])} aria-label={`Move ${item.name}`} title="Move to folder"><FolderInput /></button><button className="icon-button" disabled={busy} onClick={() => { setRenameItem(item); setRenameName(item.name); }} aria-label={`Rename ${item.name}`}><Pencil /></button><button className="icon-button danger" disabled={busy} onClick={() => setDeleteItem(item)} aria-label={`Delete ${item.name}`}>{busyKey === item.key ? <LoaderCircle className="spin" /> : <Trash2 />}</button></div></div><div><b title={item.key}>{item.name}</b><span>{formatBytes(item.size)}{item.lastModified ? ` · ${new Date(item.lastModified).toLocaleDateString()}` : ""}</span></div>{kind === "audio" ? <button className="secondary-button" disabled>Ready for Video Studio</button> : kind === "file" ? <button className="secondary-button" disabled>Unsupported file</button> : <button className="secondary-button" onClick={() => onCompose({ name: item.name, url: item.url, previewUrl: item.url, type: kind })}>Use in post</button>}</article>; })}</div>}
    ...
    {createOpen && <PromptModal title="Create an asset folder" body={createParentId ? `This folder will be created inside ${projects.find((project) => project.id === createParentId)?.name ?? "the selected folder"}.` : "This folder will be created at the root. Name it after an app, brand, campaign, music mood, or anything else."} value={projectName} placeholder="e.g. Product launch" confirmLabel="Create folder" busy={projectBusy} error={projectError} onChange={setProjectName} onClose={() => setCreateOpen(false)} onConfirm={() => void createProject()} />}
    ...
    {moveProject && <MoveMediaFolderModal project={moveProject} projects={visibleProjects} value={moveParentId} busy={projectBusy} error={projectError} onChange={setMoveParentId} onClose={() => setMoveProject(null)} onConfirm={() => void moveFolderTo(moveProject.id, moveParentId)} />}
    {moveFolderSelection.length > 0 && <MoveMediaFoldersModal folders={moveFolderSelection} projects={visibleProjects} value={moveFolderParentId} busy={projectBusy} error={moveFolderError} onChange={setMoveFolderParentId} onClose={() => setMoveFolderSelection([])} onConfirm={() => void moveSelectedFolders()} />}
    ...
    {moveItems.length > 0 && <MoveMediaModal items={moveItems} projects={visibleProjects} value={moveProjectId} busy={busyKey === "move-selection"} error={moveError} onChange={setMoveProjectId} onClose={() => setMoveItems([])} onConfirm={() => void move()} />}
...
function ComposerMediaLibrary({ onClose, onSelect, imagesOnly = false, jpegOnly = false, title = "Choose from your library" }: { onClose: () => void; onSelect: (media: ComposerMedia) => void; imagesOnly?: boolean; jpegOnly?: boolean; title?: string }) {
  ...
  const folderOptions = flattenMediaProjects(projects);
