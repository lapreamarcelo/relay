...
function MoveMediaModal({ items, projects, value, busy, error, onChange, onClose, onConfirm }: { items: MediaObject[]; projects: MediaProject[]; value: string; busy: boolean; error: string; onChange: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  ...
  return <div className="modal-layer"><button className="modal-scrim" disabled={busy} onClick={onClose} aria-label="Cancel moving media" /><form className="prompt-modal move-media-modal" role="dialog" aria-modal="true" aria-labelledby="move-media-title" onSubmit={(event) => { event.preventDefault(); onConfirm(); }}><header><div><p className="eyebrow">Media library</p><h2 id="move-media-title">Move {multiple ? `${items.length} files` : "media"} to a folder</h2><p>Relay will preserve references from scheduled posts and creative projects.</p></div><button type="button" className="icon-button" disabled={busy} onClick={onClose} aria-label="Close"><X /></button></header><div className="move-media-file"><span><FolderInput /></span><div><b>{multiple ? `${items.length} selected files` : items[0]?.name}</b><small>{multiple ? items.slice(0, 3).map((item) => item.name).join(", ") + (items.length > 3 ? ` +${items.length - 3} more` : "") : "Choose its new R2 folder"}</small></div></div><label htmlFor="move-media-folder">Destination folder</label><select id="move-media-folder" autoFocus value={value} onChange={(event) => onChange(event.target.value)}>{canMoveTo("unfiled") && <option value="unfiled">Unsorted</option>}{projects.filter((project) => canMoveTo(project.id)).map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select>{error && <p className="auth-error" role="alert">{error}</p>}<footer><button type="button" className="secondary-button" disabled={busy} onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy || !value}>{busy ? <LoaderCircle className="spin" /> : <FolderInput />}{busy ? `Moving ${items.length}…` : `Move ${multiple ? `${items.length} files` : "file"}`}</button></footer></form></div>;
}
...
function MoveMediaFolderModal({ project, projects, value, busy, error, onChange, onClose, onConfirm }: { project: MediaProject; projects: MediaProject[]; value: string; busy: boolean; error: string; onChange: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  ...
  return <div className="modal-layer"><button className="modal-scrim" disabled={busy} onClick={onClose} aria-label="Cancel moving folder" /><form className="prompt-modal move-media-modal" role="dialog" aria-modal="true" aria-labelledby="move-media-folder-title" onSubmit={(event) => { event.preventDefault(); onConfirm(); }}><header><div><p className="eyebrow">Media library</p><h2 id="move-media-folder-title">Move folder</h2><p>Drop folders onto each other in the library, or choose a destination here.</p></div><button type="button" className="icon-button" disabled={busy} onClick={onClose} aria-label="Close"><X /></button></header><div className="move-media-file"><span><FolderInput /></span><div><b>{project.name}</b><small>{project.count} item{project.count === 1 ? "" : "s"} · {project.kind === "music" ? "music" : "media"} folder</small></div></div><label htmlFor="move-media-folder">Destination folder</label><select id="move-media-folder" autoFocus value={value} onChange={(event) => onChange(event.target.value)}><option value="root">Root folders</option>{options.map(({ project: candidate, depth }) => <option value={candidate.id} key={candidate.id}>{mediaProjectOptionLabel(candidate.name, depth)}</option>)}</select>{error && <p className="auth-error" role="alert">{error}</p>}<footer><button type="button" className="secondary-button" disabled={busy} onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy}>{busy ? <LoaderCircle className="spin" /> : <FolderInput />}{busy ? "Moving folder…" : "Move folder"}</button></footer></form></div>;
}
...
function MediaView({ onCompose, publishingDefaults = defaultPublishingDefaults, demoMode = false, initialItems = [], initialMusicItems = [], initialProjects = [] }: { onCompose: (media: ComposerMedia) => void; publishingDefaults?: PublishingDefaults; demoMode?: boolean; initialItems?: MediaObject[]; initialMusicItems?: MediaObject[]; initialProjects?: MediaProject[] }) {
  ...
  const createProject = async () => {
    ...
    try { const response = await fetch("/api/v1/media/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: projectName, kind: assetKind, parentId: createParentId }) }); const payload = await response.json() as { data?: MediaProject; error?: string }; if (!response.ok || !payload.data) throw new Error(payload.error || "Could not create the project"); setProjects((current) => [...current, payload.data!]); setProjectId(payload.data.id); setCursor(null); setHistory([]); setCreateOpen(false); setProjectName(""); setCreateParentId(null); }
  ...
  const switchAssetKind = (next: "media" | "music") => { setAssetKind(next); setMediaType("all"); setSelectedItems([]); if (demoMode) setItems(next === "music" ? initialMusicItems : initialItems); setProjectId("all"); setCursor(null); setHistory([]); };
  ...
  return <div className="page page-enter"><div className="inline-heading"><div><h2>Asset library</h2><p>Keep videos, images, and licensed music in named R2 folders that you and your agents can reuse.</p></div><button className="primary-button" disabled={busyKey === "upload"} onClick={() => inputRef.current?.click()}>{busyKey === "upload" ? <LoaderCircle className="spin" /> : <Upload />} {busyKey === "upload" ? uploadProgress : `Upload to ${selectedName}`}</button><input ref={inputRef} className="visually-hidden" type="file" multiple accept={assetKind === "music" ? "audio/*" : "image/*,video/*"} onChange={(event) => void uploadFiles(event.target.files)} /></div>
    ...
    <section className="media-projects"><header><div><p className="eyebrow">R2 folders</p><h3>Choose a reusable asset folder</h3></div><div className="media-project-header-actions">{selectedProject && <><button className="secondary-button" onClick={() => openMoveFolder(selectedProject)}><FolderInput /> Move folder</button><button className="secondary-button" onClick={() => { setRenameProject(selectedProject); setRenameProjectName(selectedProject.name); setProjectError(""); }}><Pencil /> Rename folder</button><button className="danger-outline-button" onClick={() => { setDeleteProject(selectedProject); setProjectError(""); }}><Trash2 /> Delete folder</button></>}<button className="secondary-button" onClick={() => { setProjectName(""); setCreateParentId(projectId !== "all" && projectId !== "unfiled" ? projectId : null); setProjectError(""); setCreateOpen(true); }}><Plus /> New folder</button></div></header><div><button className={projectId === "all" ? "active" : ""} onClick={() => chooseProject("all")}><span><Images /></span><b>{assetKind === "music" ? "All music" : "All media"}</b><small>{assetKind === "music" ? "Every audio track" : "Every visual asset"}</small></button><button className={projectId === "unfiled" ? "active" : ""} onClick={() => chooseProject("unfiled")}><span><FolderOpen /></span><b>Unsorted</b><small>Existing and loose files</small></button>{visibleProjectTree.map(({ project, depth }) => <button className={`${projectId === project.id ? "active" : ""}${folderDropTargetId === project.id ? " folder-drop-target" : ""}`} style={{ "--folder-depth": depth } as React.CSSProperties} draggable={!projectBusy} onDragStart={(event) => startFolderDrag(event, project)} onDragOver={(event) => overFolder(event, project)} onDrop={(event) => dropFolder(event, project)} onDragEnd={() => { setDraggedProjectId(null); setFolderDropTargetId(null); }} onClick={() => chooseProject(project.id)} key={project.id}><span><Folder /></span><b>{project.name}</b><small>{project.count} item{project.count === 1 ? "" : "s"}</small></button>)}</div></section>
