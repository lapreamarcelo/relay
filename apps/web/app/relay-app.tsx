...
function MediaView({ onCompose, publishingDefaults = defaultPublishingDefaults, demoMode = false, initialItems = [], initialMusicItems = [], initialProjects = [] }: { onCompose: (media: ComposerMedia) => void; publishingDefaults?: PublishingDefaults; demoMode?: boolean; initialItems?: MediaObject[]; initialMusicItems?: MediaObject[]; initialProjects?: MediaProject[] }) {
  ...
  const switchAssetKind = (next: "media" | "music") => { setAssetKind(next); setMediaType("all"); setSelectedItems([]); setFolderQuery(""); if (demoMode) setItems(next === "music" ? initialMusicItems : initialItems); setProjectId("all"); setCursor(null); setHistory([]); };
  ...
  return <div className="page page-enter"><div className="inline-heading"><div><h2>Asset library</h2><p>Keep videos, images, and licensed music in named R2 folders that you and your agents can reuse.</p></div><button className="primary-button" disabled={busyKey === "upload"} onClick={() => inputRef.current?.click()}>{busyKey === "upload" ? <LoaderCircle className="spin" /> : <Upload />} {busyKey === "upload" ? uploadProgress : `Upload to ${selectedName}`}</button><input ref={inputRef} className="visually-hidden" type="file" multiple accept={assetKind === "music" ? "audio/*" : "image/*,video/*"} onChange={(event) => void uploadFiles(event.target.files)} /></div>
    ...
    <section className="media-projects"><header><div><p className="eyebrow">R2 folders</p><h3>Choose a reusable asset folder</h3></div><div className="media-project-header-actions">{selectedProject && <><button className="secondary-button" onClick={() => openMoveFolder(selectedProject)}><FolderInput /> Move folder</button><button className="secondary-button" onClick={() => { setRenameProject(selectedProject); setRenameProjectName(selectedProject.name); setProjectError(""); }}><Pencil /> Rename folder</button><button className="danger-outline-button" onClick={() => { setDeleteProject(selectedProject); setProjectError(""); }}><Trash2 /> Delete folder</button></>}<button className="secondary-button" onClick={() => { setProjectName(""); setCreateParentId(projectId !== "all" && projectId !== "unfiled" ? projectId : null); setProjectError(""); setCreateOpen(true); }}><Plus /> New folder</button></div></header><div className="media-project-toolbar"><label className="media-folder-search"><Search /><span className="visually-hidden">Search folders</span><input type="search" value={folderQuery} onChange={(event) => setFolderQuery(event.target.value)} placeholder="Search folders…" aria-controls="media-folder-list" />{folderQuery && <button type="button" onClick={() => setFolderQuery("")} aria-label="Clear folder search"><X /></button>}</label><div className="media-folder-view" aria-label="Folder view"><button className={folderView === "strip" ? "active" : ""} aria-pressed={folderView === "strip"} onClick={() => setFolderView("strip")}><List /> Compact</button><button className={folderView === "grid" ? "active" : ""} aria-pressed={folderView === "grid"} onClick={() => setFolderView("grid")}><Grid2X2 /> View all folders</button></div></div><div id="media-folder-list" className={`media-project-list ${folderView}`}>{!normalizedFolderQuery && <><button className={`media-system-folder${projectId === "all" ? " active" : ""}`} onClick={() => chooseProject("all")}><span><Images /></span><b>{assetKind === "music" ? "All music" : "All media"}</b><small>{assetKind === "music" ? "Every audio track" : "Every visual asset"}</small></button><button className={`media-system-folder unsorted${projectId === "unfiled" ? " active" : ""}`} onClick={() => chooseProject("unfiled")}><span><FolderOpen /></span><b>Unsorted</b><small>Existing and loose files</small></button></>}{filteredProjectTree.map(({ project, depth }) => <button className={`${projectId === project.id ? "active" : ""}${folderDropTargetId === project.id ? " folder-drop-target" : ""}`} style={{ "--folder-depth": depth } as React.CSSProperties} draggable={!projectBusy} onDragStart={(event) => startFolderDrag(event, project)} onDragOver={(event) => overFolder(event, project)} onDrop={(event) => dropFolder(event, project)} onDragEnd={() => { setDraggedProjectId(null); setFolderDropTargetId(null); }} onClick={() => chooseProject(project.id)} key={project.id}><span><Folder /></span><b>{project.name}</b><small>{project.count} item{project.count === 1 ? "" : "s"}{depth > 0 ? ` · Level ${depth + 1}` : ""}</small></button>)}</div>{normalizedFolderQuery && filteredProjectTree.length === 0 && <div className="media-folder-empty"><Search /><span><b>No folders match “{folderQuery.trim()}”</b><small>Try a shorter name or clear the search.</small></span><button className="secondary-button" onClick={() => setFolderQuery("")}>Clear search</button></div>}</section>
