...
import { CommunitySponsors, GoldSponsor, SponsorMarketplace } from "./growthcat-sponsors";
...
function ComposerMediaLibrary({ onClose, onSelect, imagesOnly = false, title = "Choose from your library" }: { onClose: () => void; onSelect: (media: ComposerMedia) => void; imagesOnly?: boolean; title?: string }) {
  const [items, setItems] = useState<MediaObject[]>([]);
  const [projects, setProjects] = useState<MediaProject[]>([]); const [projectId, setProjectId] = useState("all");
  const [cursor, setCursor] = useState<string | null>(null);
  const [history, setHistory] = useState<(string | null)[]>([]);
...
function VideoFramePicker({ videoUrl, initialOffsetMs, platform = "Video", onClose, onSelect }: { videoUrl: string; initialOffsetMs?: number; platform?: string; onClose: () => void; onSelect: (offsetMs: number) => void }) {
  ...
  return <div className="modal-layer media-picker-layer"><button className="modal-scrim" onClick={onClose} aria-label="Close frame picker" /><section className="frame-picker" role="dialog" aria-modal="true" aria-labelledby="frame-picker-title"><header><div><p className="eyebrow">{platform} cover</p><h2 id="frame-picker-title">Choose a frame</h2></div><button className="icon-button" onClick={onClose} aria-label="Close"><X /></button></header><video ref={videoRef} src={videoUrl} muted playsInline preload="auto" onLoadedMetadata={(event) => { const duration = Math.round(event.currentTarget.duration * 1_000); const initial = Math.min(initialOffsetMs ?? Math.round(duration / 2), duration); setDurationMs(duration); setOffsetMs(initial); event.currentTarget.currentTime = initial / 1_000; }} onSeeked={(event) => event.currentTarget.pause()} /><label>Frame position <span>{(offsetMs / 1_000).toFixed(1)}s</span><input type="range" min="0" max={durationMs || 1} step="100" value={Math.min(offsetMs, durationMs || 1)} onChange={(event) => position(Number(event.target.value))} /></label><footer><button className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={!durationMs} onClick={() => onSelect(offsetMs)}><Check /> Use this frame</button></footer></section></div>;
}
...
function Composer({ onClose, onSave, publishingDefaults, initialMedia = null, initialPost = null, initialText = "", initialBrandId, mode = "create" }: { onClose: () => void; onSave: (post: RelayPost, mode: ComposerMode) => Promise<boolean>; publishingDefaults: PublishingDefaults; initialMedia?: ComposerMedia | null; initialPost?: RelayPost | null; initialText?: string; initialBrandId?: string; mode?: ComposerMode }) {
  ...
  const [coverSource, setCoverSource] = useState<"instagram-library" | "instagram-frame" | "tiktok-frame" | null>(null);
  ...
  useEffect(() => () => { if (localPreviewRef.current) URL.revokeObjectURL(localPreviewRef.current); if (localCoverPreviewRef.current) URL.revokeObjectURL(localCoverPreviewRef.current); }, []);
  ...
  const addMedia = async (file: File) => {
    ...
    setInstagramCoverUrl(""); setInstagramCoverPreview(""); setInstagramThumbOffsetMs(undefined); setTiktokThumbOffsetMs(undefined); setVideoDurationMs(0);
  ...
  const chooseLibraryMedia = (item: ComposerMedia) => { if (localPreviewRef.current) URL.revokeObjectURL(localPreviewRef.current); localPreviewRef.current = null; setMedia(item); setMediaSource(null); setMediaError(""); setInstagramCoverUrl(""); setInstagramCoverPreview(""); setInstagramThumbOffsetMs(undefined); setTiktokThumbOffsetMs(undefined); setVideoDurationMs(0); };
  const removeMedia = () => { if (localPreviewRef.current) URL.revokeObjectURL(localPreviewRef.current); localPreviewRef.current = null; setMedia(null); setInstagramCoverUrl(""); setInstagramCoverPreview(""); setInstagramThumbOffsetMs(undefined); setTiktokThumbOffsetMs(undefined); setVideoDurationMs(0); };
  const uploadCover = async (file: File) => {
    if (file.type !== "image/jpeg") { setCoverError("Instagram Reel covers uploaded through Relay must be JPEG files."); return; }
    setCoverBusy(true); setCoverError(""); if (localCoverPreviewRef.current) URL.revokeObjectURL(localCoverPreviewRef.current);
    const previewUrl = URL.createObjectURL(file); localCoverPreviewRef.current = previewUrl; setInstagramCoverPreview(previewUrl);
    try {
      const signedResponse = await fetch("/api/v1/media", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: file.name, contentType: file.type }) });
      const signed = await signedResponse.json() as { uploadUrl?: string; url?: string; error?: string };
      ...
      setInstagramCoverUrl(url); setInstagramThumbOffsetMs(undefined);
    } catch (cause) { URL.revokeObjectURL(previewUrl); localCoverPreviewRef.current = null; setInstagramCoverPreview(""); setInstagramCoverUrl(""); setCoverError(cause instanceof Error ? cause.message : "Cover upload failed"); }
    finally { setCoverBusy(false); if (coverInputRef.current) coverInputRef.current.value = ""; }
  };
  const chooseCoverImage = (item: ComposerMedia) => { if (localCoverPreviewRef.current) URL.revokeObjectURL(localCoverPreviewRef.current); localCoverPreviewRef.current = null; setInstagramCoverUrl(item.url); setInstagramCoverPreview(item.previewUrl); setInstagramThumbOffsetMs(undefined); setCoverSource(null); setCoverError(""); };
