...
export async function GET(request:Request){
 ...
 return Response.json({data:{apiVersion:"v1",timelineVersion:1,automaticCaptions:Boolean(process.env.OPENAI_API_KEY),scopes,features:["video-timeline","timed-labels","video-templates","render-jobs","weekly-queues","campaign-operations","idea-inbox","brand-kits","caption-jobs","creative-analytics","posting-time-observations","campaign-recipes","writing-assistance"],limits:{clips:50,timelineDurationMs:900000,timedLabels:200,queueSlots:50,bulkPosts:100},renderWorkflow:{enqueue:"POST /api/v1/videos/render with {id,async:true}",poll:"GET /api/v1/videos/jobs?id=…",cancelOrRetry:"PATCH /api/v1/videos/jobs",completion:"Use completed job.renderedUrl in POST /api/v1/posts"}}});
}
