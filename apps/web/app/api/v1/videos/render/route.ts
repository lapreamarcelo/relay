...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as { id?: unknown; async?: boolean } | null; const id = typeof body?.id === "string" ? body.id.trim() : "";
  if (!id) return Response.json({ error: "A video project id is required." }, { status: 400 });
  if (body?.async) {
    try { return Response.json(await enqueueRender(authorization.session.user.id,id),{status:202}); }
    catch(error){ return Response.json({error:error instanceof Error ? error.message : "Could not queue render."},{status:400}); }
  }
  ...
  try {
    ...
    const artifact = await renderVideoArtifactDetails({ projectId: id, sourceUrl: serialized.sourceUrl, musicUrl: serialized.musicUrl, labels: serialized.labels, timeline: serialized.timeline, targetKey: `media-projects/${folderId}/media/${serialized.name.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").slice(0, 72) || "video"}.mp4` });
    const renderedUrl=artifact.url;
    const [updated] = await sql<VideoProjectRow[]>`UPDATE "video_project" SET rendered_url=${renderedUrl}, rendered_cover_url=${artifact.coverUrl??null}, updated_at=NOW() WHERE id=${id} AND owner_id=${authorization.session.user.id} AND revision=${serialized.revision??1} RETURNING *`;
    return Response.json({ data: updated ? serializeVideoProject(updated) : { ...serialized, renderedUrl, renderedCoverUrl:artifact.coverUrl }, folder: { id: folderId, name: folderName } });
