...
const toCampaign = (row: { id: string; brand_id: string | null; name: string; color: string; status: "active" | "archived"; paused: boolean; post_count: number; created_at: string | Date; updated_at: string | Date }): Campaign => ({ id: row.id, brandId: row.brand_id ?? "", name: row.name, color: row.color, status: row.status, paused: row.paused, postCount: row.post_count, createdAt: new Date(row.created_at).toISOString(), updatedAt: new Date(row.updated_at).toISOString() });

export async function GET(request: Request) {
  const auth = await requireApiSession(request, { apiKeyScope: "posts:read" }); if (auth.response) return auth.response;
  const rows = await sql<Parameters<typeof toCampaign>[0][]>`SELECT campaign.id, campaign.brand_id, campaign.name, campaign.color, campaign.status, campaign.paused, campaign.created_at, campaign.updated_at, count(post.id)::int AS post_count FROM "campaign" campaign LEFT JOIN "post" post ON post.campaign_id = campaign.id WHERE campaign.owner_id = ${auth.session.user.id} GROUP BY campaign.id ORDER BY campaign.updated_at DESC`;
...
export async function POST(request: Request) {
  ...
  const [row] = await sql<Parameters<typeof toCampaign>[0][]>`INSERT INTO "campaign" (id, owner_id, brand_id, name, color) VALUES (${crypto.randomUUID()}, ${auth.session.user.id}, ${brandId}, ${name}, ${color(body?.color)}) RETURNING id, brand_id, name, color, status, paused, 0::int AS post_count, created_at, updated_at`;
