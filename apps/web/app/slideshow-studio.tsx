...
import {
  ArrowLeft, Check, ChevronLeft, ChevronRight, CircleAlert, Copy, ExternalLink, Images, LoaderCircle,
...
interface MediaProject { id: string; name: string; kind?: "media" | "music"; parentId?: string | null }
...
function MediaPicker({ onAdd, onClose }: { onAdd: (asset: { url: string; staged?: StagedMedia }) => void; onClose: () => void }) {
  ...
  const [uploading, setUploading] = useState(false); const [uploadProgress, setUploadProgress] = useState(""); const [dragActive, setDragActive] = useState(false); const dragDepth = useRef(0); const fileInput = useRef<HTMLInputElement>(null);
  const [source, setSource] = useState<MediaSourceTab>("library");
  const [pexelsConfigured, setPexelsConfigured] = useState<boolean | null>(null); const [pexelsQuery, setPexelsQuery] = useState("");
  const [pexelsPage, setPexelsPage] = useState(1); const [pexelsHasMore, setPexelsHasMore] = useState(false);
  ...
  const folderOptions = useMemo(() => flattenMediaProjects(projects), [projects]);
  useEffect(() => {
  ...
  useEffect(() => {
    void fetch("/api/v1/media/sources", { cache: "no-store" }).then(async (response) => {
      const payload = await response.json() as { configured?: boolean };
      setPexelsConfigured(response.ok && payload.configured === true);
    }).catch(() => setPexelsConfigured(false));
  }, []);
  ...
  const isFileDrag = (event: React.DragEvent<HTMLElement>) => Array.from(event.dataTransfer.items).some((item) => item.kind === "file") || Array.from(event.dataTransfer.types).includes("Files");
  const handleDragEnter = (event: React.DragEvent<HTMLDivElement>) => { if (uploading || !isFileDrag(event)) return; event.preventDefault(); dragDepth.current += 1; setDragActive(true); };
  const handleDragOver = (event: React.DragEvent<HTMLDivElement>) => { if (uploading || !isFileDrag(event)) return; event.preventDefault(); event.dataTransfer.dropEffect = "copy"; };
  const handleDragLeave = (event: React.DragEvent<HTMLDivElement>) => { if (!dragActive) return; event.preventDefault(); dragDepth.current = Math.max(0, dragDepth.current - 1); if (dragDepth.current === 0) setDragActive(false); };
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => { if (!dragActive && !isFileDrag(event)) return; event.preventDefault(); dragDepth.current = 0; setDragActive(false); if (!uploading) void uploadFiles(event.dataTransfer.files); };
  const loadExternal = async (append = false) => {
    if (!pexelsConfigured) { setError("Pexels is not configured. Add PEXELS_API_KEY to .env and restart Relay."); return; }
    if (!pexelsQuery.trim()) { setError("Enter something to search for on Pexels."); return; }
    const requestedPage = append ? pexelsPage + 1 : 1;
    setExternalLoading(true); setError("");
    try {
      const response = await fetch("/api/v1/media/sources", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: "pexels", query: pexelsQuery, page: requestedPage }) });
      const payload = await response.json() as { items?: ExternalImageResult[]; page?: number; hasMore?: boolean; error?: string };
      if (!response.ok) throw new Error(payload.error || "Could not load images.");
      setExternalItems((current) => append ? [...current, ...(payload.items ?? [])] : payload.items ?? []);
      setPexelsPage(payload.page ?? requestedPage); setPexelsHasMore(payload.hasMore === true);
  ...
  const showSource = (next: MediaSourceTab) => { setSource(next); setError(""); setExternalItems([]); setPexelsPage(1); setPexelsHasMore(false); };
  ...
  return <div className="modal-layer slideshow-media-layer"><button className="modal-scrim" onClick={onClose} aria-label="Close media library" /><section className="slideshow-media-picker" role="dialog" aria-modal="true" aria-labelledby="slide-media-title"><header><div><p className="eyebrow">Add media</p><h2 id="slide-media-title">Add images to the slideshow</h2><p>Imported images stay temporary until you save or render.</p></div><button className="icon-button" onClick={onClose}><X /></button></header><nav className="slideshow-source-tabs" aria-label="Image source"><button className={source === "library" ? "active" : ""} onClick={() => showSource("library")}><Images />Library & upload</button><button className={source === "pexels" ? "active" : ""} onClick={() => showSource("pexels")}><Search />Pexels</button></nav><div className="slideshow-media-body"><label className="media-project-select"><Folder />Save selected images to<select value={projectId} onChange={(event) => setProjectId(event.target.value)}><option value="all">Unsorted Media</option>{folderOptions.map(({ project, depth }) => <option value={project.id} key={project.id}>{mediaProjectOptionLabel(project.name, depth)}</option>)}</select></label>{source === "library" && <><div className={`slideshow-upload-source${dragActive ? " drag-active" : ""}`} onDragEnter={handleDragEnter} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}><div><Upload /><span><b>{dragActive ? "Drop images to upload" : "Upload from computer"}</b><small>{dragActive ? "Release to add them to the slideshow." : "Choose several images or drag them here—they’ll be added in order."}</small></span></div><input ref={fileInput} hidden type="file" accept="image/*" multiple onChange={(event) => void uploadFiles(event.target.files)} /><button className="primary-button" disabled={uploading} onClick={() => fileInput.current?.click()}>{uploading ? <LoaderCircle className="spin" /> : <Upload />}{uploading ? uploadProgress : "Choose files"}</button></div><div className="slideshow-media-label"><div><Images /><span><b>Media library</b><small>Your existing images stored in Cloudflare.</small></span></div><span>{items.length} available</span></div>{error && <p className="composer-error">{error}</p>}{loading ? <div className="media-loading"><LoaderCircle className="spin" />Loading images…</div> : <div className="slideshow-media-grid">{items.map((item) => <button key={item.key} onClick={() => onAdd({ url: item.url })}><img src={item.url} alt={item.name} /><span><Plus /> Add</span><b>{item.name}</b></button>)}{!items.length && <p>No images in this project yet. Upload some above.</p>}</div>}</>}{source === "pexels" && <>{pexelsConfigured === null ? <div className="media-loading"><LoaderCircle className="spin" />Checking Pexels configuration…</div> : pexelsConfigured ? <section className="external-source-setup"><form className="external-search pexels-search" onSubmit={(event) => { event.preventDefault(); void loadExternal(); }}><label><span>Search Pexels photos</span><input aria-label="Search Pexels" value={pexelsQuery} onChange={(event) => setPexelsQuery(event.target.value)} placeholder="Editorial workspace, coastal morning…" /></label><button className="primary-button" disabled={externalLoading}>{externalLoading ? <LoaderCircle className="spin" /> : <Search />} Search</button></form></section> : <section className="external-source-setup external-source-missing"><div className="external-source-heading"><CircleAlert /><span><b>Pexels API key required</b><small>Add <code>PEXELS_API_KEY=your_key</code> to the server’s <code>.env</code> file, then restart Relay.</small></span></div></section>}{pexelsConfigured && <>{error && <p className="composer-error">{error}</p>}{externalLoading && !externalItems.length ? <div className="media-loading"><LoaderCircle className="spin" />Loading images…</div> : providerGrid}{pexelsHasMore && <button className="secondary-button external-load-more" disabled={externalLoading} onClick={() => void loadExternal(true)}>{externalLoading ? <LoaderCircle className="spin" /> : <Plus />} Load more photos</button>}{externalItems.length > 0 && <p className="external-provider-credit">Photos provided by <a href="https://www.pexels.com" target="_blank" rel="noreferrer">Pexels</a>. Photographer credit and source links are preserved with each import.</p>}</>}</>}</div><footer><span>Select as many images as you need</span><button className="primary-button" onClick={onClose}>Done</button></footer></section></div>;
}
